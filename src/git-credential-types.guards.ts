...
export function isGitCredentialInputOutput(input: unknown): input is GitCredentialInputOutput {
  return typeof input === 'object' && input !== null && (
      ...
      ('password_expiry_utc' in input ? typeof input['password_expiry_utc'] === 'number' : true) &&
      ('oauth_refresh_token' in input ? typeof input['oauth_refresh_token'] === 'string' : true) &&
      ('url' in input ? typeof input['url'] === 'string' : true) &&
      ('wwwauth' in input ? Array.isArray(input['wwwauth']) && input['wwwauth'].every(item => typeof item === 'string') : true)
  );
}